"""ItalianPoliceAgent — live scraper dla strony Polizia di Stato.

Scrapuje komunikaty z:
https://www.poliziadistato.it/pressarea/comunicatiStampa

Filtruje te dotyczace transportu (furto, trasporto, camion, carico).
"""
...
POLIZIA_URL = "https://www.poliziadistato.it/articolo/welcome"
...
class ItalianPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Polizia di Stato dot. transportu.

    Scrapuje z: poliziadistato.it/pressarea/comunicatiStampa
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(POLIZIA_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/comunicat" in href.lower() or "/press" in href.lower():
                    parent = link.find_parent(["div", "li", "article", "section"])
                    if parent and parent not in items:
                        items.append(parent)

        self._logger.info("Polizia: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Polizia: %d transport-related events", transport_count)
        return events
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", POLIZIA_URL),
    ...
    def _extract_date(self, item) -> str:
        ...
        for cls_name in ["date", "data", "news-date"]:
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "it")
