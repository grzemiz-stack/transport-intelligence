"""HungarianPoliceAgent — live scraper dla strony Rendőrség Magyarország.

Scrapuje: https://www.police.hu/hu/hirek-es-informaciok/legfrissebb-hireink
Keywords HU: lopás, szállítás, kamion, autópálya
"""
...
POLICE_URL = "https://www.police.hu/hu/hirek-es-informaciok/legfrissebb-hireink"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"

RATE_LIMIT_SECONDS = 5.0


class HungarianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="HU", language="hu",
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        items = (soup.select("article") or soup.select("div.news-item")
                 or soup.select("div.view-content div.views-row") or soup.select("li.list-item"))
        if not items:
            for link in soup.find_all("a", href=True):
                if "/hirek/" in link["href"]:
                    p = link.find_parent(["div", "li", "article", "section"])
                    if p and p not in items:
                        items.append(p)
        self._logger.info("Police.hu: znaleziono %d elementow", len(items))
        for item in items:
        ...
        self._logger.info("Police.hu: %d transport-related events", len(events))
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            h = link_el["href"]
            href = ("https://www.police.hu" + h) if h.startswith("/") else (h if h.startswith("http") else "https://www.police.hu/" + h)
        desc_el = item.find("p")
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "hu")
