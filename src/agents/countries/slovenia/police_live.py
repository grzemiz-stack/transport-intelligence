"""SlovenianPoliceAgent — live scraper dla strony Policija Slovenije.

Scrapuje: https://www.policija.si/medijsko-sredisce/sporocila-za-javnost
Keywords SL: kraja, prevoz, tovornjak, avtocesta
"""
...
POLICE_URL = "https://www.policija.si/medijsko-sredisce/sporocila-za-javnost"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"
RATE_LIMIT_SECONDS = 5.0

class SlovenianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="SI", language="sl", source_type=SourceType.POLICE,
            source_name="Policija Slovenije", source_url=POLICE_URL,
    ...
    async def parse(self, raw_data):
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                if "/sporocila" in link["href"]:
                    p = link.find_parent(["div","li","article","section"])
                    if p and p not in items: items.append(p)
        self._logger.info("Policija.si: znaleziono %d elementow", len(items))
        for item in items:
            ...
            if link_el:
                h = link_el["href"]
                href = ("https://www.policija.si"+h) if h.startswith("/") else (h if h.startswith("http") else "https://www.policija.si/"+h)
            desc_el = item.find("p")
            desc = desc_el.get_text(strip=True) if desc_el else ""
            full = f"{title} {desc}".lower()
            if is_transport_related(full, "sl"):
                ...
                if key and key not in seen:
                    ...
                    events.append({"title":title,"description":desc,"date":date,"source_url":href,"raw_text":f"{title}\n{desc}","country_code":"SI","language":"sl","source_name":"Policija Slovenije","trust_score":1.0,"is_official":True,"source_type":SourceType.POLICE.value,"timestamp":date or datetime.utcnow().isoformat(),"collected_at":datetime.utcnow().isoformat()})
        self._logger.info("Policija.si: %d transport-related events", len(events))
        return events
