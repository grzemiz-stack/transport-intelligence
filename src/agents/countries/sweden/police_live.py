"""SwedishPoliceAgent — live scraper dla strony Polisen Sverige.

Scrapuje: https://polisen.se/aktuellt/polisens-nyheter/
Keywords SE: stöld, transport, lastbil, motorväg
"""
...
POLICE_URL = "https://polisen.se/aktuellt/polisens-nyheter/"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"

RATE_LIMIT_SECONDS = 5.0


class SwedishPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="SE", language="sv",
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                if "/nyheter/" in link["href"] or "/polisens-nyheter/" in link["href"]:
                    p = link.find_parent(["div", "li", "article", "section"])
                    if p and p not in items:
                        items.append(p)
        self._logger.info("Polisen.se: znaleziono %d elementow", len(items))
        for item in items:
        ...
        self._logger.info("Polisen.se: %d transport-related events", len(events))
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            h = link_el["href"]
            href = ("https://polisen.se" + h) if h.startswith("/") else (h if h.startswith("http") else "https://polisen.se/" + h)
        desc_el = item.find("p")
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "sv")
