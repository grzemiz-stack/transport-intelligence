"""RomanianPoliceAgent — live scraper dla strony Poliția Română.

Scrapuje komunikaty z:
https://www.politiaromana.ro/ro/stiri

Filtruje te dotyczace transportu (furt, transport, camion, marfă).
"""
...
POLITIA_URL = "https://www.politiaromana.ro/ro/stiri"
...
class RomanianPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Politia Romana dot. transportu.

    Scrapuje z: politiaromana.ro/ro/stiri
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(POLITIA_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/stiri/" in href or "/stire/" in href:
                    parent = link.find_parent(["div", "li", "article", "section"])
                    if parent and parent not in items:
                        items.append(parent)

        self._logger.info("Politia Romana: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Politia Romana: %d transport-related events", transport_count)
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://www.politiaromana.ro" + href
            elif not href.startswith("http"):
                href = "https://www.politiaromana.ro/" + href
            result["link"] = href
        else:
            result["link"] = POLITIA_URL
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", POLITIA_URL),
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "ro")
