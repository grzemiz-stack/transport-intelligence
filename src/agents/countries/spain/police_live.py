"""SpanishPoliceAgent — live scraper dla strony Policía Nacional.

Scrapuje komunikaty z:
https://www.policia.es/prensa/noticias.html

Filtruje te dotyczace transportu (robo, transporte, camión, carga).
"""
...
POLICIA_URL = "https://www.policia.es/_es/comunicacion_salaprensa.php"
...
class SpanishPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Policía Nacional dot. transportu.

    Scrapuje z: policia.es/prensa/noticias.html
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(POLICIA_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/prensa/" in href or "/noticia" in href.lower():
                    parent = link.find_parent(["div", "li", "article", "tr", "section"])
                    if parent and parent not in items:
                        items.append(parent)

        self._logger.info("Policia.es: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Policia.es: %d transport-related events", transport_count)
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://www.policia.es" + href
            elif not href.startswith("http"):
                href = "https://www.policia.es/" + href
            result["link"] = href
        else:
            result["link"] = POLICIA_URL
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", POLICIA_URL),
    ...
    def _extract_date(self, item) -> str:
        ...
        for cls_name in ["date", "fecha", "news-date"]:
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "es")
