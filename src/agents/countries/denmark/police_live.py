"""DanishPoliceAgent — live scraper dla strony Politi Danmark.

Scrapuje komunikaty z: https://politi.dk/nyheder
Keywords DK: tyveri, transport, lastbil, motorvej
"""
...
POLICE_URL = "https://politi.dk/nyheder"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"

RATE_LIMIT_SECONDS = 5.0


class DanishPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="DK", language="da",
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        items = (soup.select("article") or soup.select("div.news-item")
                 or soup.select("div.views-row") or soup.select("li.list-item"))
        if not items:
            for link in soup.find_all("a", href=True):
                if "/nyheder/" in link["href"]:
                    p = link.find_parent(["div", "li", "article", "section"])
                    if p and p not in items:
                        items.append(p)
        self._logger.info("Politi.dk: znaleziono %d elementow", len(items))
        for item in items:
        ...
        self._logger.info("Politi.dk: %d transport-related events", len(events))
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            h = link_el["href"]
            href = ("https://politi.dk" + h) if h.startswith("/") else (h if h.startswith("http") else "https://politi.dk/" + h)
        desc_el = item.find("p")
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "da")
