"""UkrainianPoliceAgent — live scraper dla strony MVS Ukrajiny.

Scrapuje: https://mvs.gov.ua/uk/press-center/news
Keywords UK: крадіжка, транспорт, вантажівка, автострада
"""
...
POLICE_URL = "https://mvs.gov.ua/uk/press-center/news"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"
RATE_LIMIT_SECONDS = 5.0

class UkrainianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="UA", language="uk", source_type=SourceType.POLICE,
            source_name="MVS Ukrajiny", source_url=POLICE_URL,
    ...
    async def parse(self, raw_data):
        ...
        events, seen = [], set()
        items = soup.select("article") or soup.select("div.news-item") or soup.select("div.card") or soup.select("div.item") or soup.select("div.list-item") or soup.select("li.list-item")
        if not items:
            for link in soup.find_all("a", href=True):
                if "/news/" in link["href"] or "/press-center/" in link["href"]:
                    p = link.find_parent(["div","li","article","section"])
                    if p and p not in items: items.append(p)
        self._logger.info("MVS.ua: znaleziono %d elementow", len(items))
        for item in items:
            ...
            if link_el:
                h = link_el["href"]
                href = ("https://mvs.gov.ua"+h) if h.startswith("/") else (h if h.startswith("http") else "https://mvs.gov.ua/"+h)
            desc_el = item.find("p")
            desc = desc_el.get_text(strip=True) if desc_el else ""
            full = f"{title} {desc}".lower()
            if is_transport_related(full, "uk"):
                ...
                if key and key not in seen:
                    ...
                    events.append({"title":title,"description":desc,"date":date,"source_url":href,"raw_text":f"{title}\n{desc}","country_code":"UA","language":"uk","source_name":"MVS Ukrajiny","trust_score":1.0,"is_official":True,"source_type":SourceType.POLICE.value,"timestamp":date or datetime.utcnow().isoformat(),"collected_at":datetime.utcnow().isoformat()})
        self._logger.info("MVS.ua: %d transport-related events", len(events))
        return events
