"""UKPoliceAgent — live scraper dla strony Metropolitan Police.

Scrapuje komunikaty z:
https://www.met.police.uk/news/

Filtruje te dotyczace transportu (theft, cargo, truck, lorry, HGV, freight).
"""
...
class UKPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Metropolitan Police dot. transportu.

    Scrapuje z: met.police.uk/news/
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(MET_POLICE_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        self._logger.info("Met Police: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Met Police: %d transport-related events", transport_count)
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://news.met.police.uk" + href
            elif not href.startswith("http"):
                href = "https://news.met.police.uk/" + href
            result["link"] = href
        else:
            result["link"] = MET_POLICE_URL
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", MET_POLICE_URL),
    ...
    def _extract_date(self, item) -> str:
        ...
        for cls_name in ["date", "news-date", "published"]:
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "en")
