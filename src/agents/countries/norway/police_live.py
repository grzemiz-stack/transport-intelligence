"""NorwegianPoliceAgent — live scraper dla strony Politiet Norge.

Scrapuje: https://www.politiet.no/nyheter-og-presse
Keywords NO: tyveri, transport, lastebil, motorvei
"""
...
POLICE_URL = "https://www.politiet.no/nyheter-og-presse"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"

RATE_LIMIT_SECONDS = 5.0


class NorwegianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="NO", language="no",
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                if "/nyheter-og-presse/" in link["href"] or "/nyhet/" in link["href"]:
                    p = link.find_parent(["div", "li", "article", "section"])
                    if p and p not in items:
                        items.append(p)
        self._logger.info("Politiet.no: znaleziono %d elementow", len(items))
        for item in items:
        ...
        self._logger.info("Politiet.no: %d transport-related events", len(events))
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            h = link_el["href"]
            href = ("https://www.politiet.no" + h) if h.startswith("/") else (h if h.startswith("http") else "https://www.politiet.no/" + h)
        desc_el = item.find("p")
    ...
    def _extract_date(self, item) -> str:
        ...
        for cls in ["date", "dato", "news-date"]:
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "no")
