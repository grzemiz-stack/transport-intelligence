"""BulgarianPoliceAgent — live scraper dla strony MVR Bulgarija.

Scrapuje: https://www.mvr.bg/press
Keywords BG: кражба, транспорт, камион, автомагистрала
"""
...
POLICE_URL = "https://www.mvr.bg/press"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"
RATE_LIMIT_SECONDS = 5.0

class BulgarianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="BG", language="bg", source_type=SourceType.POLICE,
            source_name="MVR Bulgarija", source_url=POLICE_URL,
    ...
    async def parse(self, raw_data):
        ...
        self._logger.info("MVR.bg: znaleziono %d elementow", len(items))
        for item in items:
            ...
            if link_el:
                h = link_el["href"]
                href = ("https://www.mvr.bg"+h) if h.startswith("/") else (h if h.startswith("http") else "https://www.mvr.bg/"+h)
            desc_el = item.find("p")
            desc = desc_el.get_text(strip=True) if desc_el else ""
            full = f"{title} {desc}".lower()
            if is_transport_related(full, "bg"):
                ...
                if key and key not in seen:
                    ...
                    events.append({"title":title,"description":desc,"date":date,"source_url":href,"raw_text":f"{title}\n{desc}","country_code":"BG","language":"bg","source_name":"MVR Bulgarija","trust_score":1.0,"is_official":True,"source_type":SourceType.POLICE.value,"timestamp":date or datetime.utcnow().isoformat(),"collected_at":datetime.utcnow().isoformat()})
        self._logger.info("MVR.bg: %d transport-related events", len(events))
        return events
