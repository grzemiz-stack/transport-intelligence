"""CzechPoliceAgent — live scraper dla strony Policie ČR.

Scrapuje komunikaty z:
https://www.policie.cz/zpravy.aspx

Filtruje te dotyczace transportu (krádež, doprava, kamion, náklad).
"""
...
POLICIE_URL = "https://www.policie.cz/zpravy.aspx"
...
class CzechPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Policie CR dot. transportu.

    Scrapuje z: policie.cz/zpravy.aspx
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(POLICIE_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        items = (
            ...
            or soup.select("div.zprava")
        )

        if not items:
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/zprav" in href.lower() or "/clanek" in href.lower():
                    parent = link.find_parent(["div", "li", "article", "section"])
                    if parent and parent not in items:
                        items.append(parent)

        self._logger.info("Policie.cz: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Policie.cz: %d transport-related events", transport_count)
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://www.policie.cz" + href
            elif not href.startswith("http"):
                href = "https://www.policie.cz/" + href
            result["link"] = href
        else:
            result["link"] = POLICIE_URL
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", POLICIE_URL),
    ...
    def _is_transport_related(self, text: str) -> bool:
        return is_transport_related(text, "cs")
