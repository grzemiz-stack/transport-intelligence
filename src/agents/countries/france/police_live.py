"""FrenchPoliceAgent — live scraper dla strony Gendarmerie Nationale.

Scrapuje komunikaty z:
https://www.gendarmerie.interieur.gouv.fr/nos-articles/actualites

Filtruje te dotyczace transportu (vol, camion, fret, autoroute).
"""
...
GENDARMERIE_URL = "https://www.gendarmerie.interieur.gouv.fr/gendinfo/actualites"
...
class FrenchPoliceAgent(BaseAgent):
    """Agent scrapujacy komunikaty Gendarmerie Nationale dot. transportu.

    Scrapuje z: gendarmerie.interieur.gouv.fr/nos-articles/actualites
    ...
    async def fetch(self) -> str:
        return await self._fetch_url(GENDARMERIE_URL)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        ...
        if not items:
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/nos-articles/" in href or "/actualites/" in href:
                    parent = link.find_parent(["div", "li", "article", "section"])
                    if parent and parent not in items:
                        items.append(parent)

        self._logger.info("Gendarmerie: znaleziono %d elementow", len(items))
        transport_count = 0
        ...
        self._logger.info("Gendarmerie: %d transport-related events", transport_count)
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://www.gendarmerie.interieur.gouv.fr" + href
            elif not href.startswith("http"):
                href = "https://www.gendarmerie.interieur.gouv.fr/" + href
            result["link"] = href
        else:
            result["link"] = GENDARMERIE_URL
    ...
    def _make_event(self, parsed: dict) -> dict:
        return {
            ...
            "source_url": parsed.get("link", GENDARMERIE_URL),
