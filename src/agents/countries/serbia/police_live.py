"""SerbianPoliceAgent — live scraper dla strony MUP Srbije.

Scrapuje: https://www.mup.gov.rs/wps/portal/sr/vesti
Keywords SR: krađa, transport, kamion, autoput
"""
...
POLICE_URL = "https://www.mup.gov.rs/wps/portal/sr/vesti"
USER_AGENT = "TransportIntelligence/1.0 (Research Bot; kontakt@example.com)"
RATE_LIMIT_SECONDS = 5.0

class SerbianPoliceAgent(BaseAgent):
    def __init__(self, event_queue=None):
        super().__init__(
            country_code="RS", language="sr", source_type=SourceType.POLICE,
            source_name="MUP Srbije", source_url=POLICE_URL,
    ...
    async def parse(self, raw_data):
        ...
        for item in items:
            ...
            if link_el:
                h = link_el["href"]
                href = ("https://www.mup.gov.rs"+h) if h.startswith("/") else (h if h.startswith("http") else "https://www.mup.gov.rs/"+h)
            desc_el = item.find("p")
            desc = desc_el.get_text(strip=True) if desc_el else ""
            full = f"{title} {desc}".lower()
            if is_transport_related(full, "sr"):
                ...
                if key and key not in seen:
                    ...
                    events.append({"title":title,"description":desc,"date":date,"source_url":href,"raw_text":f"{title}\n{desc}","country_code":"RS","language":"sr","source_name":"MUP Srbije","trust_score":1.0,"is_official":True,"source_type":SourceType.POLICE.value,"timestamp":date or datetime.utcnow().isoformat(),"collected_at":datetime.utcnow().isoformat()})
        self._logger.info("MUP.rs: %d transport-related events", len(events))
        return events
