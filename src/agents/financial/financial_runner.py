"""Financial Runner — uruchamia europejskie financial scrapers.

Uzycie:
    python -m src.agents.financial.financial_runner --country DE --once
    python -m src.agents.financial.financial_runner --country PL --once
    python -m src.agents.financial.financial_runner --all --once
    python -m src.agents.financial.financial_runner --all --once --dry-run
"""
...
logger = logging.getLogger("financial_runner")

FINANCIAL_REGISTRY: dict[str, dict[str, type]] = {}

RATE_LIMIT_BETWEEN_COUNTRIES = 10.0


def _register_financial_agents():
    """Rejestruje dostepne financial agenty."""
    from src.agents.financial.companies_house_uk import UKCompaniesHouseAgent
    from src.agents.financial.debt_registries import DebtRegistryAgent
...
async def _save_events_to_db(events: list[dict]) -> int:
    ...
    async with async_session() as session:
        ...
        source_type = events[0].get("source_type", "financial") if events else "financial"
...
async def run_financial_agent(country: str, agent_name: str, once: bool = True, dry_run: bool = False):
    """Uruchamia financial agenta z pelnym pipeline'em."""
    _register_financial_agents()

    country = country.upper()
    if country not in FINANCIAL_REGISTRY:
        logger.error("Nieznany kraj: %s. Dostepne: %s", country, list(FINANCIAL_REGISTRY.keys()))
        sys.exit(1)

    agents = FINANCIAL_REGISTRY[country]
    ...
    if not dry_run:
        try:
            ...
            logger.debug("Failed to update agent_status: %s", e)

    await agent.stop()


async def run_country_financial(country: str, once: bool = True, dry_run: bool = False):
    """Uruchamia wszystkie financial agenty dla danego kraju."""
    _register_financial_agents()
    country = country.upper()
    ...
    for agent_name in agents:
        try:
            await run_financial_agent(country=country, agent_name=agent_name, once=once, dry_run=dry_run)
        except Exception as e:
            logger.error("Financial agent %s/%s failed: %s", country, agent_name, e)


ALL_FINANCIAL_COUNTRIES = ["DE", "PL", "GB", "FR", "AT", "EU"]


async def run_all_financial(once: bool = True, dry_run: bool = False):
    """Uruchamia financial scrapers dla wszystkich krajow z rate limiting."""
    _register_financial_agents()
    logger.info("=" * 70)
    logger.info("START: all_financial_scrapers (%d krajow)", len(ALL_FINANCIAL_COUNTRIES))
    logger.info("=" * 70)

    total_start = time.time()
    total_errors = 0

    for i, country in enumerate(ALL_FINANCIAL_COUNTRIES):
        if country not in FINANCIAL_REGISTRY:
            logger.warning("Brak financial agentow dla %s — skip", country)
            continue

        logger.info("--- [%d/%d] %s financial ---", i + 1, len(ALL_FINANCIAL_COUNTRIES), country)
        try:
            await run_country_financial(country=country, once=once, dry_run=dry_run)
        except Exception as e:
            logger.error("Financial agents %s failed: %s", country, e)
            total_errors += 1

        if i < len(ALL_FINANCIAL_COUNTRIES) - 1:
    ...
    total_elapsed = time.time() - total_start
    logger.info("=" * 70)
    logger.info("ALL FINANCIAL DONE: %.1fs, %d errors", total_elapsed, total_errors)
    logger.info("=" * 70)


def main():
    parser = argparse.ArgumentParser(description="Transport Intelligence Financial Runner")
    parser.add_argument("--country", help="Kod kraju ISO 2-literowy (np. DE, PL, GB, FR, AT)")
    parser.add_argument("--agent", help="Nazwa agenta (np. insolvency, licenses, companies_house)")
    parser.add_argument("--all", action="store_true",
                        help="Uruchom financial scrapers dla wszystkich krajow")
    ...
    if args.all:
        asyncio.run(run_all_financial(once=args.once, dry_run=args.dry_run))
        ...
        asyncio.run(run_country_financial(
            country=args.country,
            once=args.once,
            dry_run=args.dry_run,
        ))
    else:
        parser.error("Podaj --country [--agent] lub --all")
