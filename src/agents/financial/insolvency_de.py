"""GermanInsolvencyAgent — scraper fuer insolvenzbekanntmachungen.de.

Scrapuje ogloszenia o upadlosciach firm transportowych z oficjalnego
portalu niemieckich ogloszen o niewypłacalności.
"""
...
SEARCH_URL = "https://www.insolvenzbekanntmachungen.de/cgi-bin/bl_suche.pl"
...
TRANSPORT_KEYWORDS = [
    ...
    "spediteur", "fuhrunternehmen", "transportgewerbe",
    "fernverkehr", "nahverkehr", "lastkraftwagen",
]

SEARCH_TERMS = [
    "Transport", "Spedition", "Logistik",
    "Güterverkehr", "Güterkraftverkehr",
]

RATE_LIMIT_SECONDS = 5.0


class GermanInsolvencyAgent(BaseAgent):
    """Agent scrapujacy insolvenzbekanntmachungen.de — niemieckie ogloszenia upadlosciowe.

    Przeszukuje oficjalny portal ogloszen o niewypłacalności w Niemczech
    ...
    async def fetch(self) -> str:
        """Pobiera wyniki wyszukiwania z insolvenzbekanntmachungen.de dla slow kluczowych transportowych."""
        all_html_parts = []
        for term in SEARCH_TERMS:
            form_data = {
                ...
                "Ession9": "",
                "Ession10": "",
                "btnSuche": "Suche+starten",
                "Name": term,
            }
            try:
                html = await self._fetch_url(SEARCH_URL, method="POST", data=form_data)
    ...
    async def parse(self, raw_data: str) -> list[dict]:
        """Parsuje wyniki wyszukiwania ogloszen upadlosciowych."""
        events = []
        seen_titles = set()
        ...
        for html_part in raw_data.split("\n<!-- SEPARATOR -->\n"):
            ...
            items = (
                ...
                or soup.select("table.result tr")
            ...
            if not items:
                for link in soup.find_all("a", href=True):
                    href = link.get("href", "")
                    if any(kw in href.lower() for kw in ["detail", "bekanntmachung", "edikte"]):
        ...
        self._logger.info("DE Insolvency: %d transport-related events", len(events))
        return events
    ...
    def _parse_item(self, item) -> dict | None:
        ...
        if link_el:
            href = link_el["href"]
            if href.startswith("/"):
                href = "https://www.insolvenzbekanntmachungen.de" + href
            elif not href.startswith("http"):
                href = "https://www.insolvenzbekanntmachungen.de/" + href
            result["link"] = href
        else:
            result["link"] = SEARCH_URL
        ...
        # Try to extract insolvency type (Eröffnung, Abweisung, etc.)
        text = item.get_text()
        for typ in ["Eröffnung", "Abweisung", "Einstellung", "Aufhebung", "Restschuldbefreiung"]:
            if typ.lower() in text.lower():
                result["insolvency_type"] = typ
                break

        # Try to extract court name
        court_pattern = re.search(r"(Amtsgericht|AG)\s+[\w\-äöüÄÖÜß]+", text)
        if court_pattern:
    ...
    def _make_event(self, parsed: dict) -> dict:
        ...
        description = parsed.get("description", "")
        if parsed.get("insolvency_type"):
            description = f"[{parsed['insolvency_type']}] {description}"
        if parsed.get("court"):
        ...
        return {
            ...
            "source_url": parsed.get("link", SEARCH_URL),
